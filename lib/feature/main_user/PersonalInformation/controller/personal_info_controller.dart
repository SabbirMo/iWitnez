...
class PersonalInfoController extends StateNotifier<PersonalInfoState> {
  ...
  void updateFullName(String name) {
    state = state.copyWith(
      userInfo: state.userInfo.copyWith(fullName: name),
    );
  }
  ...
  void updateDateOfBirth(String dob) {
    state = state.copyWith(
      userInfo: state.userInfo.copyWith(dateOfBirth: dob),
    );
  }

  void updateGender(String gender) {
    state = state.copyWith(
      userInfo: state.userInfo.copyWith(gender: gender),
    );
  }

  void updateAddress(String address) {
    state = state.copyWith(
      userInfo: state.userInfo.copyWith(address: address),
    );
  }
